 ...
 * @since   LatMath 1.0
 */
public class EMinMaxW2 extends BasicLaterationAlgorithm implements Releasable {
	...
	for (int j = 0; j < 4; j++) {
		for (int i = 0; i < anchors.length; i++) {		
			weights[j] += Math.pow(p[j].distance(anchors[i]) - ranges[i], 2); // W-2 (best for complete scenario)
		}
