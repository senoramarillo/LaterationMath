...
/**
 * Robust least-squared multilateration with GEO-N initialization (RLSM-Geo-N).
 * <p>
 ...
 * @since   LatMath 1.0
 */
public final class RLSMGEON extends BasicLaterationAlgorithm implements Releasable {
    ...
    public static Point2d multilaterate(Point2d[] anchors, double[] ranges) {
        ...
        for (int k = s; k <= M; k++) {
            ...
            for (int i = 0; i < binom; i++) {
                ...
                Point2d pe = geon.localize(tmpAnchors, tmpRanges, null, null, i, i);
                if (pe != null) {
                    intermediatePositions.add(pe);
                }
        ...
        return pts.length > 0 ? Point2d.centerOfMass(pts) : null;
    }
